			...
			<Button
				onClick={() =>
					toast({
						position: 'top',
						duration: 5000,
						render: () => <Toast title="Error" description="account created." status="error" />
					})
				}
			>
				Toast
				...
				<Table
					...
					select={(register) => (
						<Flex flex={1} justify="end">
							<Select placeholder="Gender" size="lg" w="auto" {...register('gender')}>
								<option>male</option>
					...
					filters={(user, watch) => {
						return user
							...
							.filter((user) => (watch('gender') ? user.gender === watch('gender') : user))
					}}
					effects={(watch) => [watch('gender')]}
					settings={{
						show: [5]
					}}
				/>
